...
const inkVertexKey = (v) => `${v[0]},${v[1]},${v[2]}`;
...
// belongs to.
function classifyInk(polygons, camera, cols, rows, cellAspect) {
  const tris = [];
...
for (const [label, turn, tilt] of [
  ...
  const r = classifyInk(polys, camera, cols, rows, cellAspect);
  console.log(`\n=== ${label} (turn ${turn}, tilt ${tilt}) ===`);
  console.log("tris:", r.triCount, "unique edges:", r.edgeCount);
  console.log("stats:", r.stats);
  console.log("kept:", r.kept.length, "dropped:", r.dropped.length);
}
