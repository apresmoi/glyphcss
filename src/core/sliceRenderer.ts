import type { RenderState, CubeFace, WallsMask, GridContext, Voxel } from "./types";
...
interface FaceKey { axis: PlaneAxis; plane: number; face: CubeFace; }
...
interface FaceData { key: FaceKey; buffer: FaceBuffer; }
...
type HoleFill = {
  /** mask used for host planning (may include allow holes) */
  mask: Uint8Array;
  /** cells that may be painted when buffer.ids==0 */
  allowMask: Uint8Array | null;
};

const holeFillVariants = (buffer: FaceBuffer, nextLayer: FaceBuffer | null): HoleFill[] => {
  ...
  // nextLayer coords are offset by minRow/minCol.
...
const runRects = (mask: Uint8Array, width: number, bounds: Rect, byColumn: boolean): Rect[] => {
...
const mergeAlignedRects = <T extends Rect>(rects: T[]): T[] => {
...
const emitHost = (host: Rect, buffer: FaceBuffer): Brush[] => {
  ...
  // Host can be holes-only in allowFill.
...
const verify = (brushes: Brush[], buffer: FaceBuffer, allowMask: Uint8Array | null, paletteIds: Map<string, number>): boolean => {
  const { width, height } = buffer;
...
const buildSlicePlan = (faceData: FaceData, nextLayer: FaceBuffer | null): SlicePlan => {
...
const FACE_ORDER = new Map<CubeFace, number>(CUBE_FACES.map((face, index) => [face, index] as const));
const NEXT_LAYER_STEP: Record<CubeFace, number> = {
  t: 1,
  fr: 1,
  fl: 1,
  b: -1,
...
const buffersEqual = (a: FaceBuffer | null, b: FaceBuffer | null): boolean => {
  if (a === b) return true;
  if (!a || !b) return false;
...
const buildFaceDataFromSnapshot = (snapshot: { layers: Voxel[][]; context: GridContext }): FaceData[] => {
  ...
  for (const builder of buildersList) {
    ...
    for (const cell of builder.cells) {
      ...
      if (!Number.isFinite(rRaw) || !Number.isFinite(gRaw) || !Number.isFinite(bRaw) || !Number.isFinite(aRaw)) continue;
      if (aRaw < 1) continue;
      const r = Math.max(0, Math.min(255, Math.round(rRaw * brightness)));
      const g = Math.max(0, Math.min(255, Math.round(gRaw * brightness)));
      const b = Math.max(0, Math.min(255, Math.round(bRaw * brightness)));
