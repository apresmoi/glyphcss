...
- **Polygon** (`<poly-polygon>` / `Poly`) — one polygon. The atomic primitive. Renders one atlas-backed `<div>` with `transform: matrix3d(...)`. Accepts standard DOM event handlers, classes, and styles — this is what makes polycss "DOM-native 3D" rather than "3D inside a black-box canvas".
