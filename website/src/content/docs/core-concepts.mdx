...
// Renderer — one pass, one string
const text = rasterize(ctx);

// Hit layer — the SAME camera object, so it cannot disagree
const cells = projectHotspots(hotspots, camera, grid.cols, grid.rows, grid.cellAspect);
```

Mutating `camera.rotY` and re-running both calls is all "animation" is here:
