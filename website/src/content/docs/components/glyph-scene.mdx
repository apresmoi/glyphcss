...
| `hiddenLines` | `"show" \| "hide"` | `"show"` | Hidden-line removal for wireframe (+ `charMode: "braille"`). `"hide"` depth-tests strokes against a solid surface prepass so a farther edge can't paint over a nearer one. No-op in `solid` (already depth-buffered) and `ink` (not wired) |
