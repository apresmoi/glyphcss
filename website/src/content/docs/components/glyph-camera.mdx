...
| `rotY` | `number` (degrees) | `0` | Spin around Y axis |
| `zoom` | `number` | `50` | Camera zoom — pixels per world unit (`zoom=50` → 50 px per world unit at `BASE_TILE`) |
| `distance` | `number` | `6` | Camera pull-back along the view axis (world units) |
...
- Rotation units are **degrees**, matching voxcss/three.js. `rotX=65, rotY=45` is the
  classic isometric-ish viewpoint. `zoom` is **pixels per world unit** — `zoom=50`
  means one world unit maps to 50 px.
