...
Both are modelled on the Three.js `OrbitControls` / `MapControls` pattern: the renderer (`<poly-scene>` / `<PolyScene>` / `createPolyScene`) owns the geometry and camera state; the controls component listens for pointer/wheel input and runs an optional `requestAnimationFrame` autorotate loop, calling back into the scene to mutate camera state.
...
<TabItem label="React">
```tsx
import { PolyScene, PolyMapControls, PolyMesh } from "@layoutit/polycss-react";

export function App() {
  return (
    <PolyScene rotX={90} rotY={0}>
...
`PolyOrbitControls` and `PolyMapControls` are purely additive — they attach their own pointer/wheel listeners to the scene's host element and run their own `requestAnimationFrame` loop when `animate` is on. State changes flow through the scene's normal `setOptions({ rotX, rotY, zoom })` path, so anything that observes scene state (atlas, lighting, custom-element attribute reflection) sees the same updates as if you had set them yourself.
