...
| `experimentalTextureEdgeRepair` | `boolean` | `true` | Repairs antialiased atlas pixels at shared textured polygon edges without expanding geometry. |
...
For the vanilla custom element, omit `experimental-texture-edge-repair` or set it with no value to keep edge repair enabled; set `experimental-texture-edge-repair="false"` to disable it.
...
| `experimentalTextureEdgeRepair` | `boolean` | Textured atlas edge repair. React / Vue meshes inherit the scene value, then default to `true`; vanilla meshes inherit the scene's `experimental-texture-edge-repair`. |
