...
| `polygons` | `Polygon[]` | — | Optional flat array of polygon objects rendered as direct children. Composes with JSX/slot children. |
...
| `fallback` | `ReactNode` | Rendered while `src` is loading. |
| `errorFallback` | `(error: Error) => ReactNode` | Rendered if parse fails. |
| `children` | `(polygon, index) => ReactNode` | Per-polygon render prop / scoped slot. |
