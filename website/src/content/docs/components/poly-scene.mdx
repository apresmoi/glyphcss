...
| `children` | — | — | `<poly-mesh>` / `<poly-polygon>` / `<poly-controls>` (vanilla) or `<PolyMesh>` / `<Poly>` / `<PolyControls>` (React / Vue). |

**Camera input** (drag, wheel, autorotate) is a separate, additive layer. Add a child `<poly-controls>` / `<PolyControls>` to enable any of those — see [PolyControls](/components/poly-controls).
