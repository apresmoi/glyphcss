...
| `stretch` | `number` | `1.0` | Extra X scale on top of `cellAspect` — use `0.95` to match glyphcss |
...
export function PerspectiveExample() {
  return (
    <GlyphcssScene mode="solid" cols={100} rows={30}>
      <GlyphcssPerspectiveCamera rotX={0.38} scale={0.4} distance={3} stretch={0.95} />
      <GlyphcssOrbitControls />
...
<template>
  <GlyphcssScene mode="solid" :cols="100" :rows="30">
    ...
    <GlyphcssPerspectiveCamera v-if="perspective" :rot-x="0.38" :scale="0.4" :distance="3" />
