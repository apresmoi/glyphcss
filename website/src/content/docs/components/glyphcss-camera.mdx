...
`<GlyphCamera>` (alias for `<GlyphPerspectiveCamera>`) is the canonical projection
...
<template>
  <GlyphScene mode="solid" :cols="100" :rows="30">
    ...
    <GlyphPerspectiveCamera v-if="perspective" :rot-x="0.38" :zoom="0.4" :distance="3" />
    <GlyphOrthographicCamera v-else :rot-x="0.62" :zoom="0.4" />
    <GlyphOrbitControls />
    <GlyphMesh :polygons="octa" />
  </GlyphScene>
</template>
