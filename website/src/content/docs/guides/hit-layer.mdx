...
A glyphcss-style "one DOM node per polygon" model gives you per-polygon events but
...
```tsx
import { GlyphcssScene, GlyphcssMesh, GlyphcssHotspot } from "@glyphcss/react";
import { octahedronPolygons } from "@glyphcss/core";

const shape = octahedronPolygons({ center: [0, 0, 0], size: 1, color: "#ffcc44" });

<GlyphcssScene mode="solid" cols={80} rows={24}>
  <GlyphcssMesh triangles={shape}>
    <GlyphcssHotspot at={[0, 1, 0]} onClick={() => alert("top vertex")}>
      <span className="badge">Top</span>
    </GlyphcssHotspot>
    <GlyphcssHotspot at={[0, -1, 0]} onClick={() => alert("bottom vertex")}>
