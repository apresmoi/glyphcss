...
The tree below uses `tree.glb`, which has embedded UV textures. Each triangle is an `<img>` whose source is a blob URL produced by an off-DOM canvas pass at load time.
