...
polycss loads 3D mesh files (OBJ, GLB, glTF) and renders them as DOM elements. UV textures are extracted from the file and packed into generated atlas pages; each textured polygon is an atlas-backed `<div>` that uses CSS background positioning.
...
4. Clip, draw texture pixels or shaded color fills, and export atlas pages to blob URLs via `canvas.toBlob()`.
5. Render each polygon as a `<div>` with `background-image`, `background-size`, and `background-position`.
