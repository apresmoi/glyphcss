...
This guide covers building `GlyphcssTriangle[]` arrays by hand, using the built-in
...
the vertices as a `Vec3[]` and the face indices by hand, then map to triangles.
...
const triangles: GlyphcssTriangle[] = faceIndices.map(([a, b, c]) => ({
...
The resulting `GlyphcssTriangle[]` is a flat array — no scene graph, no hierarchy.
Each element has `vertices: [Vec3, Vec3, Vec3]` and an optional `color` hex string.

## Built-in helpers

`@glyphcss/core` ships geometry generators for all common shapes. Each returns
`Polygon[]` — convert to `GlyphcssTriangle[]` for the glyphcss rasterizer, or pass
directly to `@glyphcss/react` / `@glyphcss/vue` mesh components.
...
export function App() {
  return (
    <GlyphcssScene mode="solid" cols={120} rows={36}>
      ...
      <GlyphcssMesh triangles={octa} />
      <GlyphcssMesh triangles={axes} />
    </GlyphcssScene>
  );
}
```
...
Generate arbitrary triangulated surfaces with a `for` loop. This example builds a
wavy plane by pushing two triangles per grid cell.
...
function makeGrid(): GlyphcssTriangle[] {
  ...
  for (let r = 0; r < ROWS; r++) {
    for (let c = 0; c < COLS; c++) {
      ...
      triangles.push({ vertices: [v00, v10, v11], color: "#33aaff" });
      triangles.push({ vertices: [v00, v11, v01], color: "#2299ee" });
    }
  }
  return triangles;
}
...
Each cell produces two triangles sharing the quad diagonal `v00→v11`. To animate
the wave, call `handle.dispose()` and `scene.add(makeGrid(newTime))` after each
