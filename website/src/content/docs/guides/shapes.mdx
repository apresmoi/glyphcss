...
<PolyDemo
  ...
  controls='["zoom","rotX","rotY","perspective","animate","merge"]'
  defaults='{"rotX":65,"rotY":45,"perspective":1000,"zoom":1}'
/>

## The polygon primitive

`<poly-polygon>` (vanilla) and `<Poly>` (React / Vue) render a single polygon as an atlas-backed `<div>` for UV-textured and flat-color faces. They forward standard DOM props (`onclick`, `class`, `style`, `aria-*`, etc.).
