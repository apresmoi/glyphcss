...
`<poly-polygon>` (vanilla) and `<Poly>` (React / Vue) render a single polygon as an `<img>` (UV-textured) or `<svg>` (flat color). They forward standard DOM props (`onclick`, `class`, `style`, `aria-*`, etc.).
