...
camera, so a static bake reproduces them exactly — but they are reachable only
through `compileScene` (the pure function in `glyphcss`). `@glyphcss/compile`'s
CLI, Vite plugin and `compileFile` do **not** accept them yet; call
`compileScene` directly if you need them baked.
