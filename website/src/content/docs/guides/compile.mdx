...
| Character encoding | `charMode` / `--char-mode` | ascii |
| Hidden lines | `hiddenLines` / `--hidden-lines` | show |
| Weighted ramp | `solidWeightRamp` (Node API only — it is step data, not a flag) | off |
| Smooth shading | `smoothShading` `creaseAngle` / `--smooth-shading` | off / 60 |
| Back faces | `doubleSided` | culled |
| Supersample | `supersample` | 1 |
| Semantic output | `glyphOutput` (with `sceneManifest` + `dictionary`) | visible |

`charMode`, `hiddenLines` and `solidWeightRamp` are pure functions of geometry +
camera, so a static bake reproduces them exactly. `wireframeJunctions` and
per-mesh `density`/`transparent` are **runtime-only** — the static path takes a
flat polygon list and cannot represent per-mesh detail layers.
...
domain coordinate fits an affine function of `(col, row)` within a tight
residual, the per-cell coordinate table (normally ~86% of the payload) collapses
to six fitted scalars; and when the effect covers every cell with
`blend: "replace"` at opacity 1, the base grid is skipped too. A curved or
partially covered surface fails those checks and keeps the full table.
