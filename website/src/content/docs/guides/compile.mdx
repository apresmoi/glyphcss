...
per-voice `angleN`/`originUN`/`originVN`, and `subcellRes: "2x4"`/`"ink"` — but
rejects the [volumetric branch and carve mode](/guides/effects#volumetric-fields-space-object)
explicitly (`space: "object"`, `render: "carve"`, or an active voice using
`linearZ`/a nonzero `originWN`): a march is a different, per-cell-per-frame
export design this affine-fit/coordinate-table bake can't fake. Check
`isGlyphFieldSynthStaticExportSupported(params)` before calling; a volumetric or carve patch
exports through the [interactive export](#interactive-export) instead, which
ships and evaluates the live effect runtime rather than a baked approximation.
