...
```sh
# Raw ASCII to the terminal — auto-fits the grid to the model (no cols/rows needed)
...
By default the CLI prints **raw text** (no `<pre>` — terminal-friendly), and with
no `--cols`/`--rows` it **auto-fits** the grid + zoom to the content, cropped tight.
