...
- `fieldN` — the spatial field. `radial` measures distance from the origin,
  ...
  that evolves in place over `time` rather than sliding.
...
`Ink cells`) as `{ name, params }` pairs — quick starting points for `params`.
`Cube tiles` and `Ink cells` exercise `argmax` and the ink contour mode.
