...
- `waveN` — the waveform: `sin`, `triangle`, `saw`, or `square`.
...
neighboring cells' resolved 3D coordinate. Under `render: "carve"` (below)
both are rejected — a march has no stable neighbor coordinate to difference.
...
holes, walls, hollow chambers — instead of a surface texture. It requires the
volumetric branch (`space: "object"`) and `subcellRes: "1x1"`.
...
`Ink cells`, `Menger sponge`) as `{ name, params }` pairs — quick starting
points for `params`. `Cube tiles` and `Ink cells` exercise `argmax` and the
ink contour mode; `Menger sponge` exercises the volumetric branch, voice
layers, and carve together.
...
`fieldSynth`: voice cards with live previews, per-voice mix, Stage / Mix /
Output / Lighting docks, and a live preset gallery. The full patch persists to
the URL, so a design can be shared or pasted straight into `params`.
...
per-voice `angleN`/`originUN`/`originVN`, and `subcellRes: "2x4"`/`"ink"` — all
2D. It rejects, with a specific error naming the reason: `space: "object"`
(the volumetric branch) and `render: "carve"`, plus an active voice
(`ampN > 0`) using `linearZ` or a nonzero `originWN` — all 3D-only semantics a
per-cell coordinate bake can't fake, since carve in particular needs a march
per cell per frame. Call `isGlyphFieldSynthStaticExportSupported(params)` to check before
exporting rather than duplicating this list; the `/synth` page's CodePen
button uses it to decide which export to offer. A volumetric or carve patch
still exports through the
