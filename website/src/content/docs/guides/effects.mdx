...
`render: "carve"`, three layers of three axis voices each — a scale-3, then
scale-9, then scale-27 lattice, `min`-blended so solid means "solid at every
scale") built from exactly this layer/duty/phase/volumetric machinery on a
plain cube mesh — no sponge geometry, no baked frames, plus a **Sierpinski
pyramid** preset (the same recipe, two layers, with base-2 constants), tuned
for the dedicated `pyramid` stage the [`/synth`](/synth) page offers alongside
its cube/sphere/tetrahedron shapes — an uncentered corner-tetra whose window
matches the Sierpinski recipe's own `[0, 1]³` assumption. The recipe's own
`square`-wave axis voices (not the `menger` SDF field) drive its Nyquist
floor, and that floor is duty-aware: `freq / min(duty, 1 − duty)` (two samples
per narrowest band) rather than a flat `2 × freq`. Menger sponge's finest band
needs only 94 steps under the fixed formula, well inside the 256-step cap — an
earlier, un-fixed estimate put it past ~281 and out of carve's budget
entirely (an earlier, shallower two-layer recipe shipped under this same name
before being superseded — its params live on as a params-only test fixture,
...
fixed-step marching on a deep (`iter: 3`) recursive scene. Two new presets,
**Menger SDF** and **Sierpinski SDF** (one SDF voice each, carve, `iter: 3`),
are the SDF-only patches this qualifies — the earlier Menger sponge/Sierpinski
pyramid presets are built from linear recipe voices and never qualify, so they
stay on the fixed-step path unchanged. No params control this — it applies
automatically whenever a mounted carve patch qualifies.
...
recipe. Three shipped presets turn this on:

- **Menger flow** gives the Menger sponge recipe's three X-axis voices
  (one per scale layer) the same nonzero `speed` — a shared phase-cycles-per-
  second rate keeps every scale completing cycles in lockstep instead of
  drifting apart — so the carved walls visibly sweep along one axis, while the
  Y/Z voices stay at `speed: 0`.
- **Breathing gyroid** animates the gyroid xray recipe's one voice: the
  implicit surface deforms in place rather than sliding along a single axis
  (a gyroid's three-term sum has no one "flow axis" the way a linear voice
  does), reading as an organic pulse rather than a directional sweep.
- **SDF bloom** animates the Menger SDF recipe's one voice. It deliberately
  keeps `wave: "step"` — the ONLY wave [sphere tracing](#sphere-tracing)
  qualifies — so the fractal keeps sphere-tracing while animated; `synthWave`'s
  `step` case is explicitly non-periodic (`+1` when `t >= 0`, else `-1`, no
  wraparound), so raising `speed*time` over time only ever SHRINKS the solid
  region — a one-way erosion/bloom, not a looping breathe cycle, and an
  inherent trade-off of staying inside the sphere-tracing predicate rather
  than a tuning mistake. A consumer wanting a repeating loop resets or wraps
  `time` itself.
...
`Ink cells`, `Menger sponge`, `Sierpinski pyramid`, `Gyroid xray`,
`Menger SDF`, `Sierpinski SDF`, `Menger flow`, `Breathing gyroid`,
`SDF bloom`) as `{ name, params }` pairs — quick starting points for
`params`. `Cube tiles` and `Ink cells` exercise `argmax` and the ink contour
mode; `Menger sponge` and `Sierpinski pyramid` exercise the volumetric
branch, voice layers, and carve together (the latter on the `pyramid`
stage) — `Menger sponge` is three scale layers deep, safe only since the
duty-aware Nyquist fix below; `Gyroid xray` exercises the SDF family and
`render: "xray"`; `Menger SDF` and `Sierpinski SDF` are the single-voice SDF
patches [sphere tracing](#sphere-tracing) qualifies; `Menger flow`,
`Breathing gyroid`, and `SDF bloom` are those same three volumetric recipes
with `speedN` turned on — see [Animated volumetrics](#animated-volumetrics).
All eight volumetric presets are also individually exported by object
identity (`GlyphMengerSpongePreset`, `GlyphSierpinskiPyramidPreset`,
`GlyphGyroidXrayPreset`, `GlyphMengerSdfPreset`, `GlyphSierpinskiSdfPreset`,
`GlyphMengerFlowPreset`, `GlyphBreathingGyroidPreset`,
`GlyphSdfBloomPreset`, alongside `GlyphCubeTilesPreset`), for a consumer that
needs to key off a specific preset rather than its display name.
...
- **Program-as-data**: pass a fully-formed program straight to a layer via
  ...
  React/Vue and `<glyph-effect-layer>` all accept a `program` prop/property,
