...
`Menger SDF`, `Sierpinski SDF`) as `{ name, params }` pairs — quick starting
points for `params`. `Cube tiles` and `Ink cells` exercise `argmax` and the
ink contour mode; `Menger sponge` and `Sierpinski pyramid` exercise the
volumetric branch, voice layers, and carve together (the latter on the
`pyramid` stage) — `Menger sponge` is three scale layers deep, safe only
since the duty-aware Nyquist fix below; `Gyroid xray` exercises the SDF
family and `render: "xray"`; `Menger SDF` and `Sierpinski SDF` are the
single-voice SDF patches [sphere tracing](#sphere-tracing) qualifies. All
five volumetric presets are also individually exported by object identity
(`GlyphMengerSpongePreset`, `GlyphSierpinskiPyramidPreset`,
`GlyphGyroidXrayPreset`, `GlyphMengerSdfPreset`,
`GlyphSierpinskiSdfPreset`, alongside `GlyphCubeTilesPreset`), for a consumer
that needs to key off a specific preset rather than its display name.
