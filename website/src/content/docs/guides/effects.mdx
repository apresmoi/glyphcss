...
`[0, 1]³` assumption. A **Menger sponge (depth 3)** preset ships alongside it —
the recipe's own `square`-wave axis voices (not the `menger` SDF field) drive
its Nyquist floor, and that floor is duty-aware: `freq / min(duty, 1 − duty)`
(two samples per narrowest band) rather than a flat `2 × freq`. Depth 3's
finest band needs only 94 steps under the fixed formula, well inside the
256-step cap — an earlier, un-fixed estimate put it past ~281 and out of
carve's budget entirely.
...
`Ink cells`, `Menger sponge`, `Menger sponge (depth 3)`, `Sierpinski pyramid`,
`Gyroid xray`, `Menger SDF`, `Sierpinski SDF`) as `{ name, params }` pairs —
quick starting points for `params`. `Cube tiles` and `Ink cells` exercise
`argmax` and the ink contour mode; `Menger sponge` and `Sierpinski pyramid`
exercise the volumetric branch, voice layers, and carve together (the latter
on the `pyramid` stage); `Menger sponge (depth 3)` is the same recipe one
level deeper, safe only since the duty-aware Nyquist fix below; `Gyroid xray`
exercises the SDF family and `render: "xray"`; `Menger SDF` and
`Sierpinski SDF` are the single-voice SDF patches [sphere tracing](#sphere-tracing)
qualifies. All six volumetric presets are also individually exported by
object identity (`GlyphMengerSpongePreset`, `GlyphMengerSpongeDepth3Preset`,
`GlyphSierpinskiPyramidPreset`, `GlyphGyroidXrayPreset`, `GlyphMengerSdfPreset`,
