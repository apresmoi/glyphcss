...
depth-buffered per cell) and in `ink` mode (not wired — a flat-bias attempt
regressed every convex mesh there; see `research/contour-first-text/decisions.md`).
...
`charMode`, `wireframeJunctions`, and `hiddenLines` are documented no-ops in
`ink` mode — it always picks its own fixed oriented-glyph set as plain ASCII.
