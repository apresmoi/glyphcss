...
Documented no-op outside `solid` mode, under `charMode: "halfblock"` (its
two-color-per-cell encoding has no font-weight span either), and during
active `temporalBlend` reprojection. Mirrored as a `solidWeightRamp` JS
