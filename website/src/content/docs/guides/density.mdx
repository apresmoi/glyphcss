...
- **Any camera.** Detail meshes are rendered *in place* at higher resolution
  (real world positions, scaled zoom, offset projection center), so orthographic,
  perspective, and first-person (FPV) cameras all work — foreshortening and
  cross-layer occlusion stay correct.
