...
High-level convenience function: fetches a URL, detects OBJ vs glTF/GLB/VOX by extension (`.obj`, `.gltf`, `.glb`, `.vox`), parses it, and returns a `ParseResult`.
...
Runs the shared mesh-resolution optimizer. `meshResolution: "lossless"` uses exact candidates; `meshResolution: "lossy"` also tries bounded approximate merge candidates and returns the smallest accepted polygon list.
