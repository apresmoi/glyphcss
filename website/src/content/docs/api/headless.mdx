...
## `createPolyControls(scene, options?)`
...
controls.stop();                        // pause loop + detach listeners (reversible)
controls.start();                       // resume after stop()
...
| `zoom` | `{ min, max }` | `{ min: 0.1, max: 10 }` | Zoom range clamps. |
...
### Returned `ControlsHandle`

| Method | Description |
|--------|-------------|
| `update(partial)` | Mutate options live (toggle drag/wheel/animate, change speed, etc.). |
| `start()` | Resume after `stop()`. Re-attaches listeners and re-arms the rAF if `animate` is on. |
| `stop()` | Pause the animate loop and detach input listeners. Reversible by `start()`. |
| `destroy()` | Hard teardown. Functionally identical to `stop()` in this implementation. |

### Required scene surface

`createPolyControls` reads from and writes to its scene argument via two members on `SceneHandle`:
...
Register the `<poly-scene>`, `<poly-mesh>`, `<poly-polygon>`, and `<poly-controls>` custom elements by importing the side-effect entry point:
