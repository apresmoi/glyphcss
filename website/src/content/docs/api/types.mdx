...
All types are exported from `@layoutit/polycss-react`, `@layoutit/polycss-vue`, and `@layoutit/polycss-core`.
...
```ts
type PolyTextureLightingMode = "none" | "multiply" | "screen";
```
...
interface CameraState {
  ...
  pan: number;
  tilt: number;
  perspective: number | null;  // null = parallel projection (PolyOrthographicCamera)
