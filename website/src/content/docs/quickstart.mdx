---
title: Quickstart
description: Install glyphcss and render an interactive 3D ASCII art scene in React, Vue, or vanilla JavaScript in 30 seconds.
---
...
```vue
<template>
  <GlyphCamera :rot-x="25" :zoom="50">
    <GlyphScene>
      <GlyphOrbitControls drag wheel />
      <GlyphMesh geometry="cuboctahedron">
        <GlyphHotspot id="vertex" :at="[1, 0, 0]" @click="onClick" />
      </GlyphMesh>
    </GlyphScene>
  </GlyphCamera>
</template>
...
- One absolutely-positioned `<div class="glyph-hotspot">` for each `<GlyphHotspot>`,
  re-projected through the same camera each render so it stays glued to the mesh.
