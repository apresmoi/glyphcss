/**
 * GlyphcssDemo runtime — imported once by GlyphcssDemo.astro. Lives in a standalone
...
// ── Init per GlyphcssDemo instance ──────────────────────────────────────────

function initGlyphcssDemo(demoEl: HTMLElement): void {
  if (demoEl.getAttribute('data-initialized')) return;
  demoEl.setAttribute('data-initialized', '1');

  const sceneHost = demoEl.querySelector('.glyphcss-demo__scene-host') as HTMLElement;
  const viewportEl = demoEl.querySelector('.glyphcss-demo__viewport') as HTMLElement;
  const stripEl = demoEl.querySelector('.glyphcss-demo__strip') as HTMLPreElement;
  const hitLayerEl = demoEl.querySelector('.glyphcss-demo__hit-layer') as HTMLElement;
  const controlsEl = demoEl.querySelector('.glyphcss-demo__controls') as HTMLElement | null;
  const loadingEl = demoEl.querySelector('.glyphcss-demo__loading') as HTMLElement;
  const statsEl = demoEl.querySelector('.glyphcss-demo__stats') as HTMLElement;
  const codeEls: Record<string, HTMLElement | null> = {
    vanilla: demoEl.querySelector('.glyphcss-demo__snippet[data-fw="vanilla"] code'),
    react: demoEl.querySelector('.glyphcss-demo__snippet[data-fw="react"] code'),
    vue: demoEl.querySelector('.glyphcss-demo__snippet[data-fw="vue"] code'),
  };
  ...
  function rebuildHotspotEls(): void {
    ...
    for (const h of hotspots) {
      ...
      el.className = 'glyphcss-demo__hotspot';
      el.tabIndex = 0;
  ...
  function rebuildSceneFromGeometry(): void {
    ...
    if (controlState.dragMode === 'fpv') {
      ...
      camera = createGlyphcssFirstPersonCamera({
  ...
  function fpvLookOffset(): number {
    // With the dedicated first-person camera (createGlyphcssFirstPersonCamera),
  ...
  function setTunables(partial: Partial<Tunables>): void {
    ...
    // skip in GlyphcssScene prevents the camera→sidebar echo loop; legitimate
  ...
  function setProjection(kind: 'perspective' | 'orthographic'): void {
    ...
    // For perspective mode we use createGlyphcssPerspectiveCamera (already active).
  ...
  demoEl.querySelector('.glyphcss-demo__tabs')?.addEventListener('click', (e) => {
    const btn = (e.target as HTMLElement).closest('.glyphcss-demo__tab') as HTMLElement | null;
    if (!btn) return;
    const fw = btn.dataset.fw;
    demoEl.querySelectorAll('.glyphcss-demo__tab').forEach((t) =>
      t.classList.toggle('active', (t as HTMLElement).dataset.fw === fw));
    demoEl.querySelectorAll('.glyphcss-demo__snippet').forEach((p) =>
      p.classList.toggle('glyphcss-demo__snippet--hidden', (p as HTMLElement).dataset.fw !== fw));
  });
  ...
  function updateCode(): void {
    ...
    if (codeEls.react) {
      codeEls.react.textContent = [
        ...
        'export function App() {',
        '  return (',
        `    <GlyphcssCamera rotX={${t.rotX.toFixed(2)}} scale={${t.scale.toFixed(3)}} distance={${t.distance.toFixed(2)}}>`,
        '      <GlyphcssScene>',
        '        <GlyphcssOrbitControls drag wheel />',
        `        <GlyphcssMesh geometry="${t.geometry}" />`,
        '      </GlyphcssScene>',
        '    </GlyphcssCamera>',
        '  );',
        '}',
    ...
    if (codeEls.vue) {
      codeEls.vue.textContent = [
        '<template>',
        `  <GlyphcssCamera :rot-x="${t.rotX.toFixed(2)}" :scale="${t.scale.toFixed(3)}" :distance="${t.distance.toFixed(2)}">`,
        '    <GlyphcssScene>',
        '      <GlyphcssOrbitControls drag wheel />',
        `      <GlyphcssMesh geometry="${t.geometry}" />`,
        '    </GlyphcssScene>',
...
export function initAllGlyphcssDemos(): void {
  document.querySelectorAll<HTMLElement>('.glyphcss-demo').forEach(initGlyphcssDemo);
}

document.addEventListener('astro:page-load', initAllGlyphcssDemos);
if (document.readyState !== 'loading') initAllGlyphcssDemos();
else document.addEventListener('DOMContentLoaded', initAllGlyphcssDemos);
