...
import { createGlyphMap, GlyphMapClassifiers, type GlyphMapHandle, type GlyphMapProvider, type GlyphMapView } from "@glyphcss/maps";
...
export default function MapsWorkbench() {
  ...
  const [mobilePanel, setMobilePanel] = useState<"places" | "controls" | null>(null);
  ...
  // ── Palette recolor: no live setter on a mounted raster layer's `colors`
  //    (see `packages/maps/src/widget.ts` — `addLayer`/`removeLayer` are the
  //    only layer mutators), so this removes and re-adds the SAME layer id
  //    with new colors. Tiles are already cached inside the OLD runtime and
  //    that runtime is disposed with it, so this re-fetches from the
  //    (same-origin, browser-cached) static JSON — acceptable for a palette
  //    swap, not a per-frame cost. ─────────────────────────────────────────
  useEffect(() => {
  ...
  // ── Tilt -> camera.rotX directly (sheet projections only — see
  //    mapsKit.tsx's `useViewFolder` doc). ────────────────────────────────
  const onTilt = useCallback((value: number) => {
    setTilt(value);
    const map = mapRef.current;
    if (!map || isOrbitProjectionId(projectionId)) return;
    map.scene.camera.rotX = value;
    map.scene.rerender();
  }, [projectionId]);
