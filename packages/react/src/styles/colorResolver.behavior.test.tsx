...
describe("colorResolver behavior", () => {
  ...
  describe("caching", () => {
    it("returns the same object reference on second call for hex color", () => {
      ...
      // parsePureColor returns fresh objects, but for DOM-resolved colors the cache kicks in.
      // For hex colors, parsePureColor is called each time, so we just verify consistency.
      expect(first).toEqual(second);
    });

    it("returns the same cached object for DOM-resolved named colors", () => {
      // Named colors go through the DOM probe path and get cached
      const first = resolveColor("red");
      const second = resolveColor("red");
      // Both calls should return the same cached reference
      if (first !== null) {
