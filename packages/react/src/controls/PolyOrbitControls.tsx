/**
 * <PolyOrbitControls> — Three.js-style orbit controls for polycss.
 *
 * Left-drag: rotates rotX/rotY (orbit).
 * Right-drag or Shift+left-drag: pans target along world ground plane.
 * Wheel: zooms (wheel up = zoom in).
 *
 *   <PolyCamera rotX={65} rotY={45} zoom={0.65}>
 *     <PolyScene>
 *       <PolyOrbitControls />
 *       <PolyMesh polygons={...} />
 *     </PolyScene>
...
export type { PolyControlsCamera as PolyOrbitControlsCamera };
export interface PolyOrbitControlsProps extends SharedControlsProps {}
...
export function PolyOrbitControls({
  ...
  const fireChange = (): void => {
    ...
    if (!fn) return;
    try { fn(cameraSnapshot()); } catch (err) { console.error("[polycss/react] PolyOrbitControls onChange threw:", err); }
  };
  const fireStart = (): void => {
    const fn = onInteractionStartRef.current;
    if (!fn) return;
    try { fn(cameraSnapshot()); } catch (err) { console.error("[polycss/react] PolyOrbitControls onInteractionStart threw:", err); }
  };
  const fireEnd = (): void => {
    const fn = onInteractionEndRef.current;
    if (!fn) return;
    try { fn(cameraSnapshot()); } catch (err) { console.error("[polycss/react] PolyOrbitControls onInteractionEnd threw:", err); }
  };

  const animationPausedShared = useRef({ value: false }).current;

  // ── Pointer drag: left=orbit, right/shift+left=pan ─────────────────────
  useEffect(() => {
    if (!drag) return;
