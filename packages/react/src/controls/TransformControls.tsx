...
function RotateRing({
  ...
  // Build the ring polygons. Vertex coords are in WORLD units; convert
  // CSS-axis (the user-facing rotation axis) to the WORLD axis the
  // ring is perpendicular to via the same WORLD_AXIS_FOR_CSS lookup
  // the arrows use. Radius lives in CSS-px scene space (matches arrow
  // shaft length), so divide by tileSize to feed `ringPolygons`.
  const polygons = useMemo<Polygon[]>(() => {
