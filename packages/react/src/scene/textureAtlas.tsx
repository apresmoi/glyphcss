...
export function useTextureAtlas(
  ...
  const packed = useMemo(() => packTextureAtlasPlans(plans), [plans]);
  ...
  useEffect(() => {
    ...
    Promise.all(packed.pages.map((page) => buildAtlasPage(page, textureLighting, document)))
      .then((nextPages) => {
        if (cancelled) {
