/**
 * <PolyOrbitControls> — Vue 3 Three.js-style orbit controls for polycss.
 *
 ...
 *
 *   <PolyCamera :rot-x="65" :rot-y="45" :zoom="0.65">
 *     <PolyScene>
 *       <PolyOrbitControls />
 *       <PolyMesh :polygons="..." />
 *     </PolyScene>
  ...
  setup(props, { emit }) {
    ...
    if (!ctx) {
      if (typeof console !== "undefined") {
        console.warn("[polycss] <PolyOrbitControls> must be used inside <PolyCamera>.");
      }
      return () => null;
    }
    const { store, cameraRef, cameraElRef, applyTransformDirect } = ctx;

    const cameraSnapshot = (): PolyOrbitControlsCamera => ({
