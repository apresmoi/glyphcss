...
export function useGlyphAnimation(
  ...
  const resolvedClipsComputed = computed<GlyphAnimationClip[]>(() => {
    const c = unwrapRef(clips);
    return c ?? [];
  });
  ...
  const mixerComputed = computed<GlyphAnimationMixer | null>(() => {
    void _mixerEpoch.value; // subscribe to epoch changes
    return _mixer;
  });

  const actionsComputed = computed<Record<string, GlyphAnimationAction | null>>(() => {
    void _mixerEpoch.value; // subscribe to epoch changes
    const clips_ = resolvedClipsComputed.value;
    const result: Record<string, GlyphAnimationAction | null> = {};
