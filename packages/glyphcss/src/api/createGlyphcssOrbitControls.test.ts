...
import { createGlyphcssOrbitControls } from "./createGlyphcssOrbitControls";
...
describe("createGlyphcssOrbitControls", () => {
  ...
  it("invert option reverses drag direction", () => {
    const controls = createGlyphcssOrbitControls(scene, { invert: true });
    const initialRotY = scene.camera.rotY;
  ...
  it("numeric invert factor scales drag magnitude", () => {
    const controls2x = createGlyphcssOrbitControls(scene, { invert: 2 });
    const initialRotY = scene.camera.rotY;
  ...
  it("drag disabled via option produces no rotation", () => {
    const controls = createGlyphcssOrbitControls(scene, { drag: false });
    const initialRotY = scene.camera.rotY;
