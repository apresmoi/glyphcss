...
import { createGlyphMapControls } from "./createGlyphMapControls";
...
describe("createGlyphMapControls", () => {
  ...
  it("invert option reverses orbit direction (right-drag) but NOT pan direction", () => {
    ...
    const controls = createGlyphMapControls(scene, { invert: true });
    const initialRotY = scene.camera.rotY;
  ...
  it("drag disabled via option produces no target change", () => {
    const controls = createGlyphMapControls(scene, { drag: false });
    const [tx0] = scene.camera.target;
  ...
  it("wheel disabled via option produces no scale change", () => {
    const controls = createGlyphMapControls(scene, { wheel: false });
    const initialZoom = scene.camera.zoom;
