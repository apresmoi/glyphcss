/**
 * <poly-map-controls> — declarative wrapper around `createPolyMapControls`.
 *
 ...
 * Left-drag pans target (slippy-map semantics); right-drag or Shift+left
 * orbits. The full options surface is exposed via kebab-case attributes:
 *
 *   <poly-map-controls
 *     drag
 *     wheel
...
import {
  ...
  type PolyMapControlsHandle,
  type PolyMapControlsOptions,
...
export class PolyMapControlsElement extends ELEMENT_BASE {
  static get observedAttributes(): string[] {
    return [...OBSERVED_ATTRS];
  }

  private _controls: PolyMapControlsHandle | null = null;

  private _readAnimate(): PolyMapControlsOptions["animate"] | undefined {
  ...
  private _readOptions(): PolyMapControlsOptions {
    const opts: PolyMapControlsOptions = {};
  ...
  private _attach(): void {
    ...
    this._controls = createPolyMapControls(handle, this._readOptions());
  }
