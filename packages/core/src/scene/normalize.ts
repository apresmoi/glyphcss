...
function isDevMode(): boolean {
  const g = globalThis as unknown as { __POLYCSS_DEV__?: boolean; process?: { env?: { NODE_ENV?: string } } };
  if (g.__POLYCSS_DEV__ === true) return true;
  if (g.__POLYCSS_DEV__ === false) return false;
