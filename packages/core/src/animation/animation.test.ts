...
describe("createPolyAnimationMixer", () => {
  ...
  it("clipAction by name returns an action", () => {
    const ctrl = makeController([makeClip(0, "run")]);
    const mixer = createPolyAnimationMixer(makeTarget(), ctrl);
...
describe("PolyAnimationAction lifecycle", () => {
...
describe("LoopPingPong", () => {
  it("time goes forward then backward", () => {
    ...
    const target = makeTarget();
    const mixer2 = createPolyAnimationMixer(target, ctrl);
