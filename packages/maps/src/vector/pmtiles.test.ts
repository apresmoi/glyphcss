...
function mvtWithUnknownThenPoint(): Uint8Array {
  const pbf = new PbfWriter();
  pbf.writeMessage(3, (_layer, layer) => {
    layer.writeStringField(1, "places");
    layer.writeMessage(2, (_feature, feature) => {
      feature.writeVarintField(1, 6);
      feature.writeVarintField(3, 0);
      feature.writePackedVarint(4, [9, 2048, 2048]);
    }, null);
    layer.writeMessage(2, (_feature, feature) => {
      feature.writeVarintField(1, 7);
      feature.writeVarintField(3, 1);
...
describe("Protomaps PMTiles provider", () => {
  ...
  it("skips an unknown MVT feature and continues decoding the tile", () => {
    const layers = glyphMapDecodeMVT(mvtWithUnknownThenPoint(), 0, 0, 0);
    expect(layers.places).toHaveLength(1);
    expect(layers.places[0].id).toBe("7");
    expect(layers.places[0].geometryType).toBe("point");
  });
